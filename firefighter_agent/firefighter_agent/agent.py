"""Core agent behavior for the FireFighter agent."""
...
class FireFighterAgent:
    """Encapsulates FireFighter-specific reasoning via the OpenAI Agent SDK."""
    ...
    def _get_or_create_session(self, context: RequestContext) -> Session | None:
        ...
        if isinstance(context.context_id, str):
            if context.context_id not in FireFighterAgent.sessions:
                FireFighterAgent.sessions[context.context_id] = SQLiteSession(
                    session_id=context.context_id,
                )
            session = FireFighterAgent.sessions[context.context_id]
        return session
